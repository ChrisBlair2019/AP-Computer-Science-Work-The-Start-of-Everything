	...
	public class FINALVOWELSREVISION {
		...
		private void FileInputProgramRunner() throws IOException 
		{
			...
			for(int a = 0; a < FileInput.length; a++) //Made a small change from the original method from "a<=FileInput.Length" to "a<FileInput.Length"
			{
					if(FileInput != null) 
			    {
			        ...
			        FINALVOWELSREVISION.StringProcessData(FileInput[a]);
		...
		public String[] StringInputData(BufferedReader Textreader) throws IOException //The argument for this 
		{
			...
			String StringArray[] = new String[4];						//Creates Array that handles taking the Text for the TextFile
			if(OriginalLine != null) 									//Reads until it reaches an input that is defined by the buffered Reader as "null"
			{
				StringArray[0]=OriginalLine;							//These are the building blocks for the String Array that we will use in the next method, they use the buffered reader to read the line of Input and resolve a string. So this would be the first String
				StringArray[1]=Textreader.readLine();	
				StringArray[2]=Textreader.readLine();	
				StringArray[3]=Textreader.readLine();	
		...
		public static void StringProcessData(String OriginalLine) throws IOException
		{
			...
			String StringOP = OriginalPrefix.toString(); 									//To prove StringTokenizer outputs
			String StringOS = OriginalSuffix.toString(); 	
			...
			System.out.println("This is the original String:" + " " + OriginalLine); 					//Due to the String declaration in the beginning and the nature of the method, it runs through this method with each String and forms a new String on each pass by, 
		...
		{
			FINALVOWELSREVISION Runner = new FINALVOWELSREVISION(); 	//Declaring new object, intitilze constructor due to the fact that the class has no constructor
